CartogramOperAnasisly = function (e) {
    ...
    var d = Backbone.View.extend({
        events: {
            ...
            i.yIn = g.indexl[0].name + "(" + g.yearl[0].name + "年 " + ")";
            this.pieRender(i)
...
function AddCartogramOper() {
    ...
    this.data = [];
    this.dataType = [{code: "singleYear", name: "单年份多指标"}, {code: "singleIndex", name: "单指标多年份"}];
    this.xAxisData = [{code: "area", name: "地区"}, {code: "year", name: "年份"}];
    this.cartogramType = [{code: "pie", name: "饼图"}, {code: "bar", name: "柱状图"}, {code: "doubleY", name: "双Y轴"}];
    this.init()
}
...
AddCartogramOper.prototype = {
    init: function () {
        ...
        var a = this;
        if (a.data.length == 0) {
            return layer.alert("没有可以添加统计图的数据！")
        }
        a.openWin()
    }, openWin: function () {
        var a = this;
        ...
        if ((c.cartogram != "doubleY" && c.yearl.length == 0) || (c.cartogram == "doubleY" && c.yearr.length == 0 && c.yearl.length == 0)) {
            return layer.alert("数据不完整，无法进行统计图的渲染和展示！")
        }
